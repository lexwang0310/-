...
/* -------------------------------------------*/
/**
 * @brief          从zset表中获取所有成对的字段field和值value（不知道有啥用，写来玩的）
 *
 * @param conn		已经建立的链接
 * @param key		哈希表名
 * @param values 结果集
 ...
 */
/* -------------------------------------------*/
int rop_zset_nget(redisContext *conn, char *key, RFIELDS fields, int from_id, int count)
{
	int retn = 0;
	size_t i = 0;
	//RFIELDS fields = NULL;
	...
	reply = redisCommand(conn, "ZREVRANGE %s %d %d", key, from_id, from_id + count);
	if (reply == NULL || reply->type != REDIS_REPLY_ARRAY) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]ZREVRANGE %s error %s\n", key,conn->errstr);
        retn = -1;
		goto END;
	}
	retn =  reply->elements < count ? reply->elements : count;
	//fields = (RFIELDS)malloc(count * FIELD_ID_SIZE);
	
    for(; i <  retn; i ++)
    {
    	//printf("field: %s\n", (reply->element)[i]->str);
    	strcpy(fields[i], (reply->element)[i]->str);
    }    
...
int rop_get_string(redisContext* conn, const char *key, char * value)
{
    ...
    //memset(value, 0, strlen(reply->str) + 1);
...
/* -------------------------------------------*/
/**
 * @brief          向hash表中插入一条指定field的数据 key field value
 *
 * @param conn		已经建立的链接
 * @param key		哈希表名
 * @param field 字段
 ...
 */
/* -------------------------------------------*/
int rop_hset(redisContext *conn, const char *key, const char * field, const char *value)
{
	int retn = 0;
...
/* -------------------------------------------*/
/**
 * @brief          从hash表中获取所有成对的字段field和值value（不知道有啥用，写来玩的）
 *
 * @param conn		已经建立的链接
 * @param key		哈希表名
 * @param values 结果集
 ...
 */
/* -------------------------------------------*/
int rop_hnget(redisContext *conn, const char *key, RVALUES values, int val_num)
{
	int retn = 0;
	size_t count, i = 0;
	RFIELDS fields = NULL;
	...
	reply = redisCommand(conn, "HKEYS %s", key);
	if (reply == NULL || reply->type != REDIS_REPLY_ARRAY) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]HNET %s error %s\n", key,conn->errstr);
        retn = -1;
		goto END;
	}
	count =  reply->elements < val_num ? reply->elements : val_num;
	fields = (RFIELDS)malloc(count * FIELD_ID_SIZE);
    for(; i <  count; i ++)
    {
    	printf("field: %s\t", (reply->element)[i]->str);
    	memset(fields[i], 0, FIELD_ID_SIZE);
    	strcpy(fields[i], (reply->element)[i]->str);
    	retn = rop_hget(conn, key, fields[i], values[i]);
    	if(retn != 0)
    		{
    			printf("func rop_hnget() rop_hget() err: loop  %ld\n", i);
    			goto END;
    		}
    	printf("value: %s\n", values[i]);
 ...
 */
/* -------------------------------------------*/
int rop_hget(redisContext *conn, const char *key, const char * field, char *value)
{
	int retn = 0;
